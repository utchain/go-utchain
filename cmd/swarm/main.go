...
import (
	...
	"github.com/utchain/go-utchain/cmd/utils"
	"github.com/utchain/go-utchain/common"
	"github.com/utchain/go-utchain/console"
	"github.com/utchain/go-utchain/crypto"
	"github.com/utchain/go-utchain/tstclient"
	"github.com/utchain/go-utchain/internal/debug"
	...
	"github.com/utchain/go-utchain/swarm"
	bzzapi "github.com/utchain/go-utchain/swarm/api"
	swarmmetrics "github.com/utchain/go-utchain/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the UTChain API provider to use to settle SWAP payments",
...
// This init function sets defaults so cmd/swarm can run alongside gut.
func init() {
...
var app = utils.NewApp(gitCommit, "UTChain Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-utchain Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.utereum/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.utereum/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.utereum/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.utereum/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gut only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gut and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the utereum node
	...
	//register BZZ as node.Service in the utereum node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	//register within the utereum node
