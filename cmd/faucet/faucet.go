...
import (
	...
	"github.com/utchain/go-utchain/p2p/discover"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the UTChain protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for UTChain and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // UTChain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an UTChain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // UTChain protocol stack
	client *tstclient.Client   // Client connection to the UTChain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the UTChain light client protocol
...
// close terminates the UTChain connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the UTChain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somtsting funky happened, please open an issue at https://github.com/utchain/go-utchain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and UTChain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for UTChain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No UTChain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the UTChain address and profile URL.
...
// returning the username, avatar URL and UTChain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain UTChain address,
