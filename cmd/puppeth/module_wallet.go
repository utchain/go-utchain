...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gut --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gut --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --tststats \'{{.Tststats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	stats := &walletInfos{
		datadir:  infos.volumes["/root/.utereum"],
		nodePort: nodePort,
		rpcPort:  rpcPort,
		webHost:  host,
		webPort:  webPort,
