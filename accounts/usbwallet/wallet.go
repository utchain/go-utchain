...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the UTChain
...
type wallet struct {
	...
	deriveChain    utereum.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived UTChain account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain utereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
