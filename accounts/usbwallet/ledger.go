...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and UTChain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an UTChain transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "UTChain app in browser mode", w.failure
	}
	if w.offline() {
		return "UTChain app offline", w.failure
	}
	return fmt.Sprintf("UTChain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whtster the wallet and the UTChain app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// UTChain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the UTChain app's version, will fail prior to v1.0.2
...
// and returning the UTChain address located on that derivation path.
...
// ledgerVersion retrieves the current version of the UTChain wallet app running
...
// ledgerDerive retrieves the currently active UTChain address from a Ledger
...
//   UTChain address length | 1 byte
//   UTChain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the UTChain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an UTChain address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the UTChain signature and do a sanity validation
	if len(reply) != 65 {
