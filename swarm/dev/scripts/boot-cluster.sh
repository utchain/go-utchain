...
#
# The cluster contains a bootnode, a gut node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gut and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gut node with --datadir pointing at <base-dir>/gut
# and a single, unlocked account with password "gut"
start_geth_node() {
  local dir="${base_dir}/gut"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gut account address"
  fi
  ...
  start_node "gut" "${GTST_IP}" "$(which gut)" ${args[@]}
}
...
create_account() {
  ...
  gut --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
