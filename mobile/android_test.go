...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.utereum", new NodeConfig());
			node.start();
			...
			UTChainClient ec = node.getUTChainClient();
	...
	// signatures too. Regression test for go-utchain issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoguter
	...
	workspace, err := ioutil.TempDir("", "gut-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/utereum/guttest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.utereum", "github.com/utchain/go-utchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gut.aar"), "gut.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "utereum", "guttest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
