## Go UTChain Dashboard

The dashboard is a data visualizer integrated into gut, intended to collect and visualize useful information of an UTChain node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-utchain` repo), these need to be installed first:
...
For more IDE support install the `linter-eslint` package too, which finds the `.eslintrc` file, and provides real-time linting. Atom warns, that these two packages are incompatible, but they seem to work well toguter. For third-party library errors and auto-completion [flow-typed][flow-typed] is used.
